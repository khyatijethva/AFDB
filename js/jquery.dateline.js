...
(function($, undefined) {
    ...
    Markers.prototype = {
        ...
        render: function() {
            ...
            while (nextDate < endDate) {
                ...
                mt = this.markerText(date);
                if ((mt.text.length > 3 || mt.text.length == undefined) && this.content.band.dateline._bands[0].scale == 4) {
                    parseInt(mt.text);
                    if (mt.text % 3 != 0) {
                        mt.text = "";
                        class_str = 'd-marker';
                    } else {
                        mt.text = monthNames[date.getMonth()] + " " + mt.text;
                        class_str = 'd-marker marker-month';
                    }
                } else {
                    class_str = 'd-marker';
                }

                var cls = this.element.parent().parent().attr('class').split(' ');
                // console.log("cls: ", cls[1]);
                
                if(cls[1] == 'd-band-1'){
                    this.element.append($('<div>', {
                        class: class_str + (mt.plus ? ' d-plus' : '')
                    }).addClass("d-e-month").width(c.band.calcPixels(nextDate - date)/daysInMonth(date.getMonth()+1,date.getFullYear())).text(mt.text));
                    for(let j=2;j<=daysInMonth(date.getMonth()+1,date.getFullYear());j++){
                        this.element.append($('<div>', {
                            class: class_str
                        }).addClass("d-e-date").width(c.band.calcPixels(nextDate - date)/daysInMonth(date.getMonth()+1,date.getFullYear())).text(j));
                    }
                }else{
                    this.element.append($('<div>', {
                        class: class_str + (mt.plus ? ' d-plus' : '')
    ...
    $('.approved_btn').on('click', function() {
        ...
        // console.log;
        if(id == '1'){
            $('#approve_notification').css('display', 'block');
        } else if(id == '2'){
            $('.successbefore').css('display', 'block');
            $('.approve-success').css('display', 'none');
            $('.approve-rejected').css('display', 'none');
        } else {
            $('.successbefore').css('display', 'block');
            $('.approve-success').css('display', 'none');
            $('.approve-rejected').css('display', 'none');
    ...
    $('.reject_btn').on('click', function() {
        var id = $(this).attr('data-id');
        // console.log;
        // var val = $('#approve_notification').html();
        if(id == '1'){
            $('#reject_notify').css('display', 'block');
        } else if(id == '2'){
            $('.rejectbefore').css('display', 'block');
            $('.rejectafter').css('display', 'none');
            $('.approve-rejected').css('display', 'none');
        } else {
            $('.rejectbefore').css('display', 'block');
            $('.rejectafter').css('display', 'none');
    ...
    function Events(content) {
        ...
        this.topMargin = 60;
        this.lineHeight = 70;
        // console.log(content.band.layout);
    ...
    Events.prototype = {
        renderNormalEvent: function(event) {
            ...
            $('.approveview-more').on('click', function() {
                var content = event;
                var id = $(this).attr('data-id');
                console.log;
                var popcontent = content.pop2Content;
                if (id == content.id) {
                    console.log(popcontent);
                    $('#event-detail .modal-body').html('');
                    $('#event-detail .modal-body').append(popcontent);
                }
            });
            ...
            if (pos) { // skip if no position available
                if (event.elements[index]) { // cached
                    ...
                    var popContent = event.popContent;
                    cls = event.class || '';
                    ...
                    if (event.stop) { // create new duration event
                        ...
                        elmt = $('<div>', {
                            ...
                            title: ttl
                        }).html(`${event.text} ${popContent}`).prepend($('<div>', {
                            class: 'd-tape'
                        }));
                    ...
                    if (band.dateline.options.url) {
                        elmt.on('mousedown touchstart', function(e) {
                            ...
                            // console.log("contentPos.top: ", contentPos.top);
        ...
        renderOverviewEvent: function(event) {
            ...
            if (event.elements[index]) { // cached
                ...
                elmt = $('<div>', {
                    class: event.stop ? 'd-tape-pin ' : 'd-pin' + cls
                });
                event.elements[index] = elmt; // cache
            }
            ...
            if (event.stop) {       // duration event
                ...
                elmt.css({
                    ...
                    // top: Math.max(this.content.band.calcPixels(stp - strt), 1)
                });
            }
            else { // instant event
                elmt.css({
        ...
        render: function() {
            var i, ev, events = this.content.band.dateline.options.events,
                range = this.content.range,
                iBegin = _.sortedIndex(events, {
                    start: range.begin
                }, function(v) {
                    return v.start;
                }),
                iEnd = _.sortedIndex(events, {
                    start: range.end
                }, function(v) {
                    return v.start;
                });
    ...
    function Content(band) {
        ...
        this.element.on("touchstart", function(e) {
            ...
            var i, t, dl = band.dateline,
                ...
                stepFunc = function(x) {
                    dl._place(x);
                }, // define functions outside loop to satisfy JsLint
            ...
            if (evt.which === 1) {
                ...
                $(band.dateline.document).on('mousemove.dateline', function(evt) { // bind event to document, using namespace
                    ...
                    var dl = band.dateline,
                        target;
                    // console.log(dl._getCursorMs());
                    var fetched_date = new Date(dl._getCursorMs());
                    var yearDate = fetched_date.toLocaleString('en-US');
                    // console.log(yearDate);
                    var d = yearDate.split(',');
                    var y = d[0].split('/');
                    // console.log(y[2]);
                    var yr = $("<h3>").text(y[2]);
                    // console.log(yr);
                    $('.calendar-div .year-calend').html('');
                    $('.calendar-div .year-calend').append(yr);
                    ...
                    // if (Math.abs(this.prevPos - this.startPos) < 4) {
                    //     // debugger;
                    //     // we hardly moved, it's a click, go to click point
                    //     target = new Date(this.visible.begin.getTime() + band.calcMs(this.startPos - band.element.offset().left));
                    //     band.roundDate(target);
                    //     dl._animateTo(target.getTime());
                    // } else {
        ...
        }.bind(this)).mousewheel(function(evt) {
            // console.log(evt);
            ...
            if (evt.originalEvent.wheelDelta / 120 > 0) {
                ...
                evt.offsetX > evt.pageX ? console.log("left") : console.log("right")

                if (evt.offsetX > evt.pageX) {
                    if (evt.pageY > evt.pageX) {
                        this.band.dateline._place(this.startMs + this.band.calcMs(x - evt.pageY));
                        console.log("xy");
                    } else {
                        this.band.dateline._place(this.startMs + this.band.calcMs(evt.pageY - x + 50));
                        console.log("yx");
                    }
                    this.velocity *= 1 - nervous;
                    this.velocity += nervous * ((x - evt.pageY) / (evt.timeStamp - this.prevTimeStamp));
                    this.prevPos = x;
                    this.prevTimeStamp = evt.timeStamp;
                } else {
                    this.band.dateline._place(this.startMs + this.band.calcMs(evt.pageY + x));
    ...
    Content.prototype = {
        ...
        calcLeft: function(date) {
            ...
            // console.log("this.range.begin: ", this.range.begin);
        ...
        calcTop: function(date) { // check for free line; if not available, return false

            var lines = $(".d-events").children().length;
    ...
    function Band(bandInfo) {
        ...
        this.before = $('<div>', {
            class: 'd-range d-before'
        });
        this.after = $('<div>', {
            class: 'd-range d-after'
        });

        this.leftIndicator = $('<div>', {
            class: 'd-indicator d-left'
        }).mousedown(function(e) {
            this.stepLeft(e.shiftKey);
            e.preventDefault();
        }.bind(this));
        this.rightIndicator = $('<div>', {
            class: 'd-indicator d-right'
        }).mousedown(function(e) {
    ...
    $.widget("calc.dateline", {
        ...
        _create: function() {
            ...
            if (opts.begin) {
                opts.begin = createDate(opts.begin);
            }
            if (opts.end) {
                opts.end = createDate(opts.end);
            }
        ...
        _triggerChange: function() {
            this._trigger('change', null, {
                cursor: new Date(this._getCursorMs())
            });
        },
        ...
        find: function(id) {
            ...
            var found = _.find(this.options.events, function(v) {
                return v.id == id;
            });
            if (found) {
